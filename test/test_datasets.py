...
def sample_dataset_data():
    ...
    train_data = np.random.rand(n_samples, n_timesteps, n_quantities).astype(np.float32)
    test_data = np.random.rand(n_samples // 4, n_timesteps, n_quantities).astype(np.float32)
    val_data = np.random.rand(n_samples // 4, n_timesteps, n_quantities).astype(np.float32)
    timesteps = np.linspace(0, 1, n_timesteps).astype(np.float32)
...
class TestDataSourcesYaml:
    ...
    def test_data_sources_yaml_exists(self):
        ...
        assert os.path.exists("datasets/data_sources.yaml"), \
    ...
    def test_data_sources_yaml_format(self):
        ...
        assert len(data_sources) > 0, \
            "data_sources.yaml should not be empty"
    ...
    def test_data_sources_urls(self):
        ...
        for dataset_name, url in data_sources.items():
            ...
            assert isinstance(url, str), \
                f"URL for {dataset_name} should be string"
    ...
    def test_expected_datasets_present(self):
        ...
        expected_datasets = [
            'osu2008', 'simple_reaction', 'simple_ode', 'lotka_volterra',
            'branca24', 'simple_primordial'
        ]
...
class TestDownloadData:
    ...
    @pytest.mark.parametrize("dataset_name", AVAILABLE_DATASETS[:2])  # Test only first 2 to avoid long downloads
...
class TestLocalDatasets:
    ...
    def test_local_dataset_structure(self, dataset_name):
        ...
        with h5py.File(dataset_path, 'r') as f:
            ...
            assert len(val_data.shape) == 3, \
                f"Val data in {dataset_name} should be 3D"
            
            # Check consistent dimensions
            assert train_data.shape[1] == test_data.shape[1] == val_data.shape[1], \
    ...
    def test_local_dataset_timesteps(self, dataset_name):
        ...
        with h5py.File(dataset_path, 'r') as f:
            if 'timesteps' in f:
                ...
                assert timesteps.shape[0] == train_data.shape[1], \
...
class TestCheckAndLoadData:
    ...
    @pytest.mark.parametrize("dataset_name", LOCAL_DATASETS[:3])  # Test first 3 local datasets
    def test_check_and_load_data_valid_dataset(self, dataset_name):
        ...
        try:
            ...
            assert len(result) == 6, \
                "check_and_load_data should return 6 elements"
            
            (train_data, test_data, val_data), params, timesteps, n_train_samples, data_info, labels = result
            ...
            assert isinstance(n_train_samples, int)
            ...
            # Check consistent dimensions
            assert train_data.shape[1] == test_data.shape[1] == val_data.shape[1] == timesteps.shape[0]
            assert train_data.shape[2] == test_data.shape[2] == val_data.shape[2]
...
class TestCreateDataset:
    ...
    def test_create_dataset_with_single_array(self, sample_dataset_data, temp_dataset_dir):
    ...
    def test_create_dataset_with_tuple_data(self, sample_dataset_data, temp_dataset_dir):
        ...
        try:
            ...
            # Check that dataset was created
            dataset_path = os.path.join(temp_datasets_dir, "test_dataset_tuple", "data.hdf5")
            assert os.path.exists(dataset_path), "Dataset file was not created"
    ...
    def test_create_dataset_with_parameters(self, sample_dataset_data, temp_dataset_dir):
        ...
        try:
            ...
            dataset_path = os.path.join(temp_datasets_dir, "test_dataset_params", "data.hdf5")
            assert os.path.exists(dataset_path), "Dataset file was not created"
    ...
    def test_create_dataset_with_labels(self, sample_dataset_data, temp_dataset_dir):
        ...
        try:
            ...
            dataset_path = os.path.join(temp_datasets_dir, "test_dataset_labels", "data.hdf5")
            assert os.path.exists(dataset_path), "Dataset file was not created"
            
            with h5py.File(dataset_path, 'r') as f:
                assert 'labels' in f
                stored_labels = [label.decode('utf-8') for label in f['labels'][:]]
