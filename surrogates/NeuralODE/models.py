...
class NeuralODE(torch.nn.Module):
    ...
    def augmented_forward(self, x): # same as forward but without t input (for calculating derivatives)
        if self.tanh_reg:
            return self.reg_factor * torch.tanh(self.mlp(x) / self.reg_factor)
        else:
            return self.mlp(x)
...
class ModelWrapper(torch.nn.Module):
    def __init__(self, real_vars: int = 29,
        ...
        self.ode = NeuralODE(self.z_vars, self.z_vars, n_hidden=ode_hidden, layer_width=ode_width, tanh_reg=tanh_reg, show=False)
    ...
    def decoder_loss(self, x_dot: torch.Tensor): # L2 term in T.Grassi
        zdot_grad_dec = torch.autograd.functional.jvp(self.decoder, self.z_pred, self.z_dot)[1]
    ...
    def ode_batch_hessian_loss(self, x, reverse_mode=True):
        if reverse_mode:
            return torch.abs(vmap(vmap(jacfwd(jacrev(self.ode.augmented_forward))))(x)).sum()
        else:
            return torch.abs(vmap(vmap(jacrev(jacfwd(self.ode.augmented_forward))))(x)).sum()
    ...
    def mass_conservation_loss(self, x_true, x_pred):
        ...
        return torch.sum(torch.abs(mass_function(x_true) - mass_function(x_pred)), dim=-1).mean()
