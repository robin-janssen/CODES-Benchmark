...
def load_data_from_files():
    ...
    for i in range(500):
        ...
        dataset[i,:,:] = torch.Tensor(np.loadtxt('/export/home/isulzer/tgrassi-latent_ode_paper/chemistry/outputs/chemistry_' + num + '.dat'))
    return dataset
...
def load_test_data():
    ...
    for i in range(50):
        num = (4 - len(str(i))) * '0' + str(i)
        dataset[i,:,:] = torch.Tensor(np.loadtxt('/export/home/isulzer/tgrassi-latent_ode_paper/chemistry/test_outputs/chemistry_' + num + '.dat'))
    return dataset
...
def plot_scatter(filepath, x_true, x_pred):
    ...
    at = AnchoredText(f"RMSE = {round(rmse(x_true, x_pred).item(), 3)}", prop=dict(size=15), frameon=True, loc='upper left')
...
def plot_relative_error(filepath, rel_error: torch.Tensor):
    ...
    logbins = np.logspace(np.max((np.log10(bins[0]), np.log10(10e-8))),np.log10(bins[-1]),len(bins))
    ...
    at = AnchoredText(f"std. dev. = {torch.std(rel_error):.3E}", prop=dict(size=10), frameon=True, loc='upper left')
    at.patch.set_boxstyle("round,pad=0.,rounding_size=0.2")
    ax[1].add_artist(at)
    ...
    ax[2].set_yscale("log")
    at = AnchoredText(f"mean = {rel_error.mean():.3E}", prop=dict(size=10), frameon=True, loc='upper right')
    at.patch.set_boxstyle("round,pad=0.,rounding_size=0.2")
    ax[2].add_artist(at) 
...
def nth_derivative(f: torch.Tensor, at: torch.Tensor, order: int):
    ...
    for i in range(order - 1):
        gradient = torch.autograd.grad(f, at, create_graph=True, retain_graph=True)[0]
        f = gradient.sum()
        derivatives = torch.vstack((derivatives, gradient[None]), )
    return derivatives
